---
name: llm-linter
description: Apply selected conventions after implementation by matching the task to focused prompts and asking which to read. Use when the user asks to lint, align, clean up, apply conventions, write a commit message, or polish completed work.
---
...
This file is a **registry**, not the rules. Each entry below points to a markdown prompt file on disk. Your job is to pick the relevant entries, get the user's approval on which to read, then read and apply them.
...
2. **Propose, do NOT read yet.** Present a **numbered list** of the registry entries you think are relevant. For each: its name and a one-line reason it applies. Do not read the prompt files at this stage.
...
3. **Wait for approval.** The user replies with the numbers they want (or picks the "Read all of the above" number). Read **only** the approved entries' prompt files.

4. **Apply.** Follow each prompt file's instructions against the relevant code, commit, or artifact. Then briefly report what you changed and which prompts you applied.

## Registry

Each entry: **name**, `path`, **tags**, and a short description. Add new prompts by appending another `---`-delimited block in the same shape.
...
`../../prompts/typescript-inference.md`
...
`../../prompts/tanstack-query-data-fetching.md`
